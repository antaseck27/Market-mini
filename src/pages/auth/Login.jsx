...
import { Link, useLocation, useNavigate } from "react-router-dom";
import { signInWithEmailAndPassword } from "firebase/auth";
import { auth, db } from "../../firebase"; // <-- db doit être exporté dans ton fichier firebase
...
export default function Login() {
  ...
  // 👑 Admin unique (peut venir du .env : VITE_ADMIN_EMAIL)
  const ADMIN_EMAIL =
    // (import.meta.env.VITE_ADMIN_EMAIL || "admin@vitemarket.com").toLowerCase();
    (import.meta.env.VITE_ADMIN_EMAIL || "antalissa10@gmail.com").toLowerCase();
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const cred = await signInWithEmailAndPassword(auth, inputEmail, pwd);
      ...
      // 2) Sinon: on lit le rôle Firestore
      ...
      try {
        ...
        // Si la lecture échoue, on laisse role="buyer"
      }
      ...
      if (role === "seller" || role === "vendeur") {
        ...
        nav(from, { replace: true }); // acheteur par défaut
      }
    } catch (e) {
      setErr(mapErr(e.code));
    } finally {
